...
- ✅ 不修改、不替换你的 `codex` 本体：通过 `codex-zh` 包装器工作，随时可卸载
...
```bash
codex-zh                       # 中文 TUI
codex-zh exec "列出当前目录"    # 非交互中文输出
codex-zh --help | less         # 中文帮助
codex-zh-doctor                # 健康检查
```

> 安装脚本会克隆本仓库到 `~/.codex-code-zh-cn`，安装 `node-pty` 依赖，
> 在 `~/.local/bin` 建立 `codex-zh` / `codex-zh-doctor` 软链，
> 并写入 `~/.codex/config.toml` 的桌面语言设置。
> 如果 `~/.local/bin` 不在 PATH 中，请手动加入。
...
3. **最长匹配优先**：短词不会抢先吃掉长句；`Starting MCP servers (2/5): ...` 这类动态文本用 `${...}` 模式匹配。
4. **语法区保护**：命令列表行、`--flag`、`/command`、反引号示例和长文段只做短语翻译，
   不会把 `--model` 变成 `--模型`。
5. **版本无关**：包装器不修改二进制，Codex 升级后重跑安装脚本即可继续使用。
