...
class Matcher {
  ...
  _compilePattern(entry) {
    // "OpenAI Codex (v${version})" -> /OpenAI Codex \(v(.+?)\)/uy
    // No `^` anchor: sticky lastIndex already fixes the start, and UI lines
    // often carry a leading "⚠ " or "• " before the pattern text.
    ...
    for (let i = 0; i < parts.length; i += 1) {
      ...
      if (i < names.length) {
        // 最后一个占位符后面没有固定后缀时用贪婪匹配，否则会只吃一个字符。
        const isTrailing = i === names.length - 1 && !parts[i + 1];
        src += isTrailing ? '(.+)' : '(.+?)';
      }
    }
